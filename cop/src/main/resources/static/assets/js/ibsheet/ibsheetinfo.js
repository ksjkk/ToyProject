...
function createIBSheet(sheetid, width, height, locale) {
    ...
    // font-size 일괄변경으로 인한 sheet 높이(고정인경우) 약간 조정. by hyungyu,Song@ildong
...
function IBS_InitSheet(sheet, info) {
    	...
    	sheet.Down2Excel = function(param){
    		...
    		try {
    			...
    			if(!newParam.FileName){
    				...
    				if(sheetTitle != "")newParam.FileName += "_" + sheetTitle;
    				newParam.FileName += "_" + $.datepicker.formatDate("ymmdd", curDate) + curDate.getHours() + curDate.getMinutes() + curDate.getSeconds();    				
    			}
    		} catch (e) {
    			// ignore error.
    			newParam = param;
    		}
    		newParam.FileName = replaceAll(newParam.FileName,'.','_');
    		originalFn.call(sheet, newParam);
    	};
    }
    
    // 붙여넣기 옵션 일괄 수정. by hyungyu@infac 
    /*
		-1	붙여넣기 기능 사용 안함
		0	선택 되어있는 해당 셀에 붙여넣기 처리
		1	선택 되어있는 셀을 시작점으로 하여 붙여 넣기 처리 (\t : 컬럼 구분자 \r\n : 행 구분자)
		2	1 설정의 방법을 기본으로 하고, 붙여넣을 행이 모자른 경우에 자동으로 행을 추가하며 붙여넣기 처리
		3	행을 추가하면서 붙여넣기 처리 (\t : 컬럼 구분자 \r\n : 행 구분자)
		4	선택 영역의 크기와 관계없이 클립보드 내용 붙여넣기 처리
     */
    sheet.SetClipPasteMode(2);

    // sheet 붙여넣기시 엑셀업로드와 같이 Edit 불가필드도 값이 채워지도록 하도록 함. 
    var currentColumnsInfo = {};
    var originBefore = window[sheet.id+"_OnBeforePaste"];
    var originAfter = window[sheet.id+"_OnAfterPaste"];
    window[sheet.id+"_OnBeforePaste"] = function (){
    	// 붙어넣기전에 non-editable cell release.
    	if(originBefore) originBefore.call(window);
    	if(!sheet.GetEditable()){
    		if(window.console) console.log("sheet.GetEditable():" + sheet.GetEditable());
    		return;
    	}
    	
    	for(var key in sheet.Cols){
    		if(sheet.Cols[key]["Type"] != "Panel" && sheet.Cols[key]["Type"] != "Status" && sheet.Cols[key]["Type"] != "Seq" && sheet.Cols[key]["Type"] != "Img"){
    			if(sheet.Cols[key].Hidden == 0){
    				currentColumnsInfo[sheet.Cols[key].SaveName] = {ColumnIdx:sheet.Cols[key].SaveName, Edit:sheet.Cols[key].Edit, InsertEdit:sheet.Cols[key].InsertEdit, UpdateEdit:sheet.Cols[key].UpdateEdit};
    				sheet.Cols[key].Edit = 1;
    				sheet.Cols[key].InsertEdit = 1;
    				sheet.Cols[key].UpdateEdit = 1;
    			}
    		}
    	}
    };
	window[sheet.id+"_OnAfterPaste"] = function (){
		// 붙어넣기전에 edit
		if(originAfter) originAfter.call(window);
		
		for(var key in sheet.Cols){
			if(currentColumnsInfo[sheet.Cols[key].SaveName]){
				sheet.Cols[key].Edit = currentColumnsInfo[sheet.Cols[key].SaveName].Edit;
				sheet.Cols[key].InsertEdit = currentColumnsInfo[sheet.Cols[key].SaveName].InsertEdit;
				sheet.Cols[key].UpdateEdit = currentColumnsInfo[sheet.Cols[key].SaveName].UpdateEdit;
			}
		}
	};
	//그룹사권한사용자 처리[2019.05.21]
	// sheet 로딩시 DoSearch, GetSearchData, DoSave 함수에 대해 Override 하여 _applEnterCd 값이 존재하면 넘기도록 처리. 
	if(!isEmpty(window._applEnterCd)){
    	var originalFnDoSearch = sheet.DoSearch;
    	sheet.DoSearch = function(url, param, sync, append){
    		var newParam = param || "";
    		try {
    			if(newParam.indexOf("applEnterCd") < 0){
    				if(typeof _applEnterCd !== 'undefined') {
	    				if(_applEnterCd != null && _applEnterCd != "" && _applEnterCd != "null"){
	    					newParam = newParam + "&applEnterCd="+_applEnterCd;
	    				}
    				}
    			}
    		} catch (e) {
 ...
    			newParam = param;
    		}
    		originalFnDoSearch.call(sheet, url, newParam, sync, append);
    	};
    	var originalFnGetSearchData = sheet.GetSearchData;
    	sheet.GetSearchData = function(url, param){
    		var newParam = param || "";
    		try {
    			if(newParam.indexOf("applEnterCd") < 0){
    				if(typeof _applEnterCd !== 'undefined') {
 ...
    			newParam = param;
    		}
    		return originalFnGetSearchData.call(sheet, url, newParam);
    	};
 
    	var originalFnDoSave = sheet.DoSave;
    	sheet.DoSave = function(url, param, Cal, Quest){
    		var newParam = param || "";
    		try {
    			if(newParam.indexOf("applEnterCd") < 0){
    				if(typeof _applEnterCd !== 'undefined') {
 ...
    			newParam = param;
    		}
    		originalFnDoSave.call(sheet, url, newParam, Cal, Quest);
    	};
	}
}
